...
test("8XY0 sets VX = VY", () => {
	...
	expect(mem[REGISTERS_OFFSET + 0xa]).toBe(0x34);
});
...
test("8XY1 sets VX = VX OR VY", () => {
	...
	chip8.step();
	const mem = new Uint8Array(chip8.getMemory().buffer);
	expect(mem[REGISTERS_OFFSET + 0xa]).toBe(0xff);
});
...
test("8XY2 sets VX = VX AND VY", () => {
	...
	chip8.step();
	const mem = new Uint8Array(chip8.getMemory().buffer);
	expect(mem[REGISTERS_OFFSET + 0xa]).toBe(0x00);
});
...
test("8XY3 sets VX = VX XOR VY", () => {
	...
	chip8.step();
	const mem = new Uint8Array(chip8.getMemory().buffer);
	expect(mem[REGISTERS_OFFSET + 0xa]).toBe(0xf0);
});
...
test("8XY4 adds VY to VX and sets VF = carry", () => {
	...
	chip8.step();
	const mem = new Uint8Array(chip8.getMemory().buffer);
	expect(mem[REGISTERS_OFFSET + 0xa]).toBe(0x10);
...
test("8XY5 subtracts VY from VX and sets VF = NOT borrow", () => {
	...
	chip8.step();
	const mem = new Uint8Array(chip8.getMemory().buffer);
	expect(mem[REGISTERS_OFFSET + 0xa]).toBe(0x20);
	expect(mem[REGISTERS_OFFSET + 0xf]).toBe(1);
});
...
test("8XY6 sets VX = VY >> 1 and stores LSB of VY in VF", () => {
	...
	chip8.step();
	const mem = new Uint8Array(chip8.getMemory().buffer);
	expect(mem[REGISTERS_OFFSET + 0xa]).toBe(0x03);
	expect(mem[REGISTERS_OFFSET + 0xf]).toBe(1);
});
...
test("8XYE shifts VX left and stores MSB in VF", () => {
	...
	chip8.step();
	const mem = new Uint8Array(chip8.getMemory().buffer);
	expect(mem[REGISTERS_OFFSET + 0xa]).toBe(0x02);
	expect(mem[REGISTERS_OFFSET + 0xf]).toBe(1);
});
